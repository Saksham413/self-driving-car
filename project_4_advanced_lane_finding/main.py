...
def prepare_out_blend_frame(blend_on_road, img_binary, img_birdeye, img_fit, line_lt, line_rt, offset_meter):
    ...
    a = 7803.3 * (math.acos(1-(3.125/(mean_curvature_meter*mean_curvature_meter))))
    cv2.putText(blend_on_road, 'steering angle: {:.02f}'.format(a), (860, 60), font, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
    ...
    d = (b/10)*1.674
    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(blend_on_road, 'distance to camera 1: {:.02f}m'.format(e), (860, 85), font, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(blend_on_road, 'speed of object 1: {:.02f}m/s'.format(b), (860, 110), font, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(blend_on_road, 'acceleration 1: {:.02f}m/s2'.format(d), (860, 135), font, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(blend_on_road, 'distance to camera 2: {:.02f}m'.format(c), (860, 160), font, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(blend_on_road, 'speed of object 2: {:.02f}m/s'.format(c), (860, 185), font, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(blend_on_road, 'acceleration 2: {:.02f}m/s2'.format(c), (860, 210), font, 0.9, (255, 255, 255), 2, cv2.LINE_AA)
...
'''
    ...
    if str_count > 400:
        cv2.putText(blend_on_road, 'DRUNK', (860, 200), font, 2, (255, 255, 255), 2, cv2.LINE_AA)
'''	
...
if __name__ == '__main__':
    ...
    if mode == 'video':
        ...
        selector = 'project'
        clip = VideoFileClip('output_v5.mp4'.format(selector)).fl_image(process_pipeline)
